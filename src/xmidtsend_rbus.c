...
int processData(wrp_msg_t * msg, rbusMethodAsyncHandle_t asyncHandle)
{
	...
	else
	{
		...
		if(outParams !=NULL)
		{
			...
			err = rbusMethod_SendAsyncResponse(asyncHandle, RBUS_ERROR_INVALID_INPUT, outParams);
			...
			ParodusInfo("outParams freed\n");
		}
		else
		{
			ParodusError("Failed to create outParams\n");
		}*/
		ParodusInfo("Free errorMsg\n");
		if(errorMsg !=NULL)
		{
			free(errorMsg);
			errorMsg = NULL;
...
void createOutParamsandSendAck(wrp_msg_t *msg, rbusMethodAsyncHandle_t asyncHandle, char *errorMsg, int statuscode)
{
	...
	rbusValue_Init(&value1);
	rbusValue_SetString(value1, msg->u.event.source);
	rbusObject_SetValue(outParams, "source", value1);
	rbusValue_Release(value1);
	...
	ParodusInfo("2 new createOutParams done\n");

	if(outParams !=NULL)
	{
		ParodusInfo("asyncHandle check\n");
		if(asyncHandle == NULL)
		{
			ParodusInfo("asyncHandle is NULL\n");
			return;
